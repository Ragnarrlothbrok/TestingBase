...
describe('<AppIcon/> component', () => {
  ...
  it('supports .color property', () => {
    ...
    const { getByTestId } = render(<ComponentToTest data-testid={testId} color={color} />);
    const svg = getByTestId(testId);
    expect(svg).toHaveAttribute('data-icon', 'default');
  ...
  it('supports .icon property', () => {
    ...
    for (const icon of Object.keys(ICONS)) {
      const testId = randomText(8);
      const { getByTestId } = render(<ComponentToTest data-testid={testId} icon={icon} />);
      const svg = getByTestId(testId);
      expect(svg).toBeDefined();
  ...
  it('supports .size property', () => {
    ...
    const { getByTestId } = render(<ComponentToTest data-testid={testId} size={size} />);
    const svg = getByTestId(testId);
  ...
  it('supports .title property', () => {
    ...
    const { getByTestId } = render(<ComponentToTest data-testid={testId} title={title} />);
    const svg = getByTestId(testId);
    expect(svg).toBeDefined();
